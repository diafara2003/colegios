...
function renderizar_tr_cursos(_item, _index) {
    ...
    _tr += '<td scope="row"><input type="text" class="input-autocomplete" onclick="iniciar_ac(\'CurTutor_' + _item.CurId + '\')" maxlength="20" id= "CurTutor_' + _item.CurId + '" value="' + _item.Nombretutor + '"/></td>';
    _tr += '<td scope="row"><input type="text" class="input-autocomplete" onclick="iniciar_ac(\'CurAuxiliar_' + _item.CurId + '\')" maxlength="20" id= "CurAuxiliar_' + _item.CurId + '"  value="' + _item.NombreAuxiliar + '"/></td>';
...
function iniciar_ac(id, placeholder, property, tipo) {
    autocomplete(id, 'Personas?tipo=1', 'PerNombres', 'PerApellidos', 'Buscar', (selected, id_ac) => {
        tutor_id = -1;
        auxiliar_id = -1;
